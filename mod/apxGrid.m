...
for ii = 1:p                                   % iteration over factor functions
  ...
  if xeqz && iscell(xg{ii})                                      % Toeplitz/BTTB
    ...
    if prod(ni)>dense_max    % empirical thresh: FFT-based MVM with 1 rhs faster
      ...
      kii = @(n) feval(f{:},hyp(v==ii),n, xii{1}(1)); % this is some sort of evil hack
      ...
      for j=1:di
          ...
          grid_tmp = xc{j};
          %xc{j} = [1:n2,flip(2:n2)]';
          xc{j} = [grid_tmp; flipud(grid_tmp(2:end))];
      end
...
function dhyp = dirder(Kg,xg,Mx,a,b)
  ...
  for i=1:p
    ...
    if isnumeric(Kg.kron(i).factor)
      ...
      for j=1:di
          %n2 = floor(ni(j)-1/2)+1;
          %xs{j} = [1:n2,n2-2*ni(j)+2:0]';
          grid_tmp = xs{j};
          %xc{j} = [1:n2,flip(2:n2)]';
          xs{j} = [grid_tmp; flipud(grid_tmp(2:end))];
      end
