...
grid_sum = strcmp(c1,'apxGridSum');
state = strcmp(c1,'apxState');
exact = ~grid && ~spars && ~state && ~grid_sum;
...
elseif grid | grid_sum                                 % C)  Grid approximations
  n = size(x,1);
  ...
  cgpar = {cgtol,cgmit};   % conjugate gradient, grid
  ...
  elseif grid_sum                                            % C)  Grid approximations
      nc = numel(cov{2});
      hyp_index = 1;
      K.mvm = @(x) (0);
      all_Kg = {};
      ...
      for ii=1:nc
          cov_ii = cov{2}{ii};
          xg_ii = cov_ii{3};
          num_hyps_ii = str2num(feval(cov{2}{ii}{:}));
          [Kg_ii,Mx_ii] = feval(cov_ii{:},hyp.cov(hyp_index:(hyp_index+num_hyps_ii-1)),x,[],deg);
          hyp_index = hyp_index + num_hyps_ii;
          
          K.mvm = @(x) (K.mvm(x) + Mx_ii*Kg_ii.mvm(Mx_ii'*x));
          all_Kg = {all_Kg{:}; Kg_ii};
          all_Mx = {all_Mx{:}; Mx_ii};
          all_xg = {all_xg{:}; xg_ii};
      end
...
%       xg2 = cov2{3};
%       
%       num_hyps1 = str2num(feval(cov1{2}{1}{:}));
%       num_hyps2 = str2num(feval(cov2{2}{1}{:}));
%       num_hyps1 = str2num(feval(cov{2}{1}{:}));
%       num_hyps2 = str2num(feval(cov{2}{2}{:}));
%       
%       [Kg1,Mx1] = feval(cov1{:},hyp.cov(1:num_hyps1),x,[],deg);  % grid cov structure, interp matrix
%       [Kg2,Mx2] = feval(cov2{:},hyp.cov((num_hyps1+1):end),x,[],deg);  % grid cov structure, interp matrix
% 
%       K.mvm = @(x) Mx1*Kg1.mvm(Mx1'*x) + Mx2*Kg2.mvm(Mx2'*x);  % mvm with covariance matrix
      K.P = @(x)x; K.Pt = @(x)x;                             % projection operations
%       K.fun = @(W) ldB2_grid_sum(W,K,Kg1,Kg2,xg1,xg2,Mx1,Mx2,cgpar,ldpar); K.Mx = {Mx1,Mx2};
    K.fun = @(W) ldB2_grid_sum(W,K,all_Kg,all_xg,all_Mx,cgpar,ldpar); 
%     K.Mx = {Mx1,Mx2};
    K.Mx = all_Mx;
  end
...
%% C)  Grid approximations =====================================================
function [ldB2,solveKiW,dW,dldB2,L,triB] = ldB2_grid_sum(W,K,all_Kg,all_xg,all_Mx,cgpar,ldpar)
  if all(W>=0)                                 % well-conditioned symmetric case
    sW = sqrt(W); msW = @(x) bsxfun(@times,sW,x);
    mvmB = @(x) msW(K.mvm(msW(x)))+x;
    solveKiW = @(r) msW(linsolve(msW(r),mvmB,cgpar{:}));
...
%       [ldB2,dhyp.cov,dW] = ldB2_cheby(W,K.mvm,dK, ldpar{2:end});
%     end
  if strncmpi(ldpar{1},'lancz',5)% stochastic estim: logdet lancz/hutchinson
    % the sign of the maxit parameter encodes whether ARPACK is used or not
    mv = ver('matlab');     % get Matlab version, ARPACK interface not in Octave
    if numel(strfind(ldpar{1},'arpack'))==0 || numel(mv)==0
      ldpar{4} = -abs(ldpar{4});
    ...
    dK = @(a,b) [];
    for ii=1:numel(all_Kg)
        Kg_ii = all_Kg{ii};
        Mx_ii = all_Mx{ii};
        xg_ii = all_xg{ii};
        dK = @(a,b) ([dK(a,b); apxGrid('dirder',Kg_ii,xg_ii,Mx_ii,a,b)]);
    end
%     dK1 = @(a,b) apxGrid('dirder',Kg1,xg1,Mx1,a,b);
%     dK2 = @(a,b) apxGrid('dirder',Kg2,xg2,Mx2,a,b);
%     dK = @(a,b) [dK1(a,b); dK2(a,b)];
    if nargout<3            % save some computation depending on required output
      ldB2 = ldB2_lanczos(W,K.mvm,dK, ldpar{[2,4,6]});
    else    
      [ldB2,dhyp.cov,dW] = ldB2_lanczos(W,K.mvm,dK, ldpar{[2,4,6]});
...
%     if nargout>3, dhyp.cov = ldB2_deriv_grid_fiedler(Kg,xg,V,ee,de,s); end
  ...
  dldB2 = @(varargin) ldB2_deriv_grid_sum(dhyp, all_Kg,all_xg,all_Mx, varargin{:});
  if ~isreal(ldB2), error('Too many negative W detected.'), end
  L = @(r) -K.P(solveKiW(K.Pt(r)));
  if nargout>5, error('triB not implemented'), end
...
% function dhyp = ldB2_deriv_grid_sum(dhyp, Kg1,Kg2,xg1,xg2,Mx1,Mx2,alpha,a,b)
%   if nargin>7, dhyp.cov = dhyp.cov-[apxGrid('dirder',Kg1,xg1,Mx1,alpha,alpha)/2; apxGrid('dirder',Kg2,xg2,Mx2,alpha,alpha)/2]; end
%   if nargin>9, dhyp.cov = dhyp.cov-[apxGrid('dirder',Kg1,xg1,Mx1,a,b)/2; apxGrid('dirder',Kg2,xg2,Mx2,a,b)/2]; end

function dhyp = ldB2_deriv_grid_sum(dhyp, all_Kg,all_xg,all_Mx,alpha,a,b)
  if nargin>4 
      tmp = [];
      for ii=1:numel(all_Kg)
          Kg_ii = all_Kg{ii};
          Mx_ii = all_Mx{ii};
          xg_ii = all_xg{ii};
          tmp = [tmp; apxGrid('dirder',Kg_ii,xg_ii,Mx_ii,alpha,alpha)/2];
      end
      dhyp.cov = dhyp.cov - tmp;
      %dhyp.cov = dhyp.cov-[apxGrid('dirder',Kg1,xg1,Mx1,alpha,alpha)/2; apxGrid('dirder',Kg2,xg2,Mx2,alpha,alpha)/2]; 
  end
  if nargin>6 
      tmp = [];
      for ii=1:numel(all_Kg)
          ...
          Mx_ii = all_Mx{ii};
          xg_ii = all_xg{ii};
          tmp = [tmp; apxGrid('dirder',Kg_ii,xg_ii,Mx_ii,a,b)/2];
      end
      dhyp.cov = dhyp.cov - tmp;
      %dhyp.cov = dhyp.cov-[apxGrid('dirder',Kg1,xg1,Mx1,a,b)/2; apxGrid('dirder',Kg2,xg2,Mx2,a,b)/2];
  end
